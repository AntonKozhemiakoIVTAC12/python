    ...
    for line_number, line in enumerate(source_code.split('\n'), start=1):
        ...
        for i, token in enumerate(tokens):
            if token == 'int' and i < len(tokens) - 1 and tokens[i + 1] in keywords:
                ...
                break
            # Добавление проверки на необъявленные идентификаторы
            if identifier_pattern.match(token) and token not in declared_variables and token not in {'int', 'for'}:
                error_messages.append(f'Error: Использование необъявленного идентификатора "{token}" на строке {line_number}.')
                error_line = line_number
                break
            ...
            # Добавление проверки на повторное объявление идентификаторов
            if token == 'int' and i < len(tokens) - 1:
                next_token = tokens[i + 1]
                if identifier_pattern.match(next_token):
                    if next_token in declared_variables:
                        error_messages.append(f'Error: Повторное объявление переменной "{next_token}" на строке {line_number}.')
                        error_line = line_number
                        break
                    else:
                        declared_variables.add(next_token)
        ...
        if '(int' in line and not re.search(r'\bfor\b\s*\(', line):
            error_messages.append(
                f'Error: Отсутствие ключевого слова "for" перед циклом на строке {line_number}.')
            error_line = line_number
